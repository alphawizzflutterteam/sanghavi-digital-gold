...
class NewVerifyMPINScreen extends StatefulWidget {



  @override
  State<NewVerifyMPINScreen> createState() => _NewVerifyMPINScreenState();
}

class _NewVerifyMPINScreenState extends State<NewVerifyMPINScreen> {
    ...
    return  Scaffold(
      ...
      bottomNavigationBar:   GestureDetector(
        onTap: () async {
          var request = http.MultipartRequest('POST', Uri.parse(baseUrl1 + 'verify_mpin'));
          request.fields.addAll({'user_id': '${App.localStorage.getString("userId").toString()}','mpin': '${pass.text}'});
          print("parameters" + request.fields.toString());
          print("ssdsd ${baseUrl1}verify_mpin");
          print(request.fields.toString() + "ssss");
          http.StreamedResponse response = await request.send();
          print(response.stream.toString());
          if (response.statusCode == 200) {
            print("working here now go to next");
            var finalResult = await response.stream.bytesToString();
            final jsonResponse = json.decode(finalResult);
            print("finalResponse here ${jsonResponse}");
